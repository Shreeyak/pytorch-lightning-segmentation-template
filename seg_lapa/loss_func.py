...
class CrossEntropy2D(nn.CrossEntropyLoss):
    ...
    Inputs:
        - inputs (Tensor): Raw output of network.
        - targets (Tensor): Ground truth, containing a class index in the range :math:`[0, C-1]` as the
        `target` for each pixel.

    Shape:
        - Input: [B, C, H, W], where C = num_classes
        - Target: [B, H, W]
        - Output: scalar.
    ...
    """
    def __init__(self, loss_per_image: bool = True,
